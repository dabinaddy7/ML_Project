...
if __name__ == "__main__":
    ...
    contamination_list = [0.001, 0.01, 0.05]
    ...
    for n_neigh in n_neighbors_list:
        for cont in contamination_list:
            model = LocalOutlierFactor(
                n_neighbors=n_neigh,
                contamination=cont,
                novelty=True, # 새로운 데이터(val) 평가를 위한 필수 설정
                n_jobs=-1
            )
            model.fit(train_X)

            # LOF의 score_samples는 정상일수록 큰 값(-)을 반환하므로 부호 반전
            ...
            print(f" - n_neighbors: {n_neigh:<3} | contamination: {cont:<5} -> Val AUROC: {val_auroc:.4f} | Val AUPR: {val_aupr:.4f}")

            if val_aupr > best_aupr:
                best_aupr = val_aupr
                best_params = {'n_neighbors': n_neigh, 'contamination': cont}

    print("\n[최종 결과] LOF 최적 파라미터:", best_params)
