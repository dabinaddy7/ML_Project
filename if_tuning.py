...
# ============================================================
# Isolation Forest 하이퍼파라미터 튜닝 파이프라인
# ============================================================
if __name__ == "__main__":
    ...
    contamination_list = [0.001, 0.005, 0.01, 0.02, 0.05]
    ...
    # 트리기반 공간 분할을 위해 300차원으로 펼침 (Flatten)
    ...
    for n_est in n_estimators_list:
        for cont in contamination_list:
            ...
            # IF의 score_samples는 정상일수록 큰 값(-)을 반환하므로 부호 반전
            ...
            print(f" - n_estimators: {n_est:<3} | contamination: {cont:<5} -> Val AUROC: {val_auroc:.4f} | Val AUPR: {val_aupr:.4f}")

            if val_aupr > best_aupr:
                best_aupr = val_aupr
                best_params = {'n_estimators': n_est, 'contamination': cont}

    print("\n[최종 결과] Isolation Forest 최적 파라미터:", best_params)
